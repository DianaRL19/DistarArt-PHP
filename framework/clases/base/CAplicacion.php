...
class CAplicacion
{
	...
	public function __construct(array $config)
	{
		...
		// definir la variable sesion
		$this->_sesion = new CSesion();

		// si esta controlautomatico activado se enciende sola la sesion
		if (isset($config["sesion"]) && $config["sesion"]["controlAutomatico"] === true) {
			$this->_sesion->crearSesion();
		}

		// definir la variable acceso
		$this->_Acceso = new CAcceso();

		// definir la variable acl
	...
	// funcion que devuelve la variable sesion
	public function sesion(): ?CSesion
	{
		return $this->_sesion;
	}

	// funcion que devuelve la variable acceso
	public function acceso(): ?CAcceso
	{
		return $this->_Acceso;
	}

	// funcion que devuelve la variable acl
